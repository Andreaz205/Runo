...
const SliderItems:FC<{ items: ISliderItem[] }> = ({items}) => {
    ...
    const swipeStart = (e: any) => {
        ...
        if (id) {
            let newTimer = setTimeout(() => {
                // push(`/products/${id}`)
                push(`/products/${id}/category/${items[0].categoryId}`)
            }, 100)
            setTimer(newTimer)
        }
    ...
    return (
        <>
            ...
            <Scrollbar slidesCount={slidesCount} slideWidth={slideWidth} sliderTrackRef={sliderTrackRef} currentSlideIndex={currentSlideIndex}/>
    </>
